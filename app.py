...
# ======================= HALAMAN ADMIN (Baru Ditambahkan) =======================
    ...
    with st.form("form_input"):
        ...
        jurusan = st.selectbox("Jurusan", ["Teknik Informatika", "Sistem Informasi", "Akuntansi", "Teknik Elektro", "Manajemen"])
        ipk = st.number_input("IPK", min_value=0.0, max_value=4.0, step=0.01)
        sks = st.number_input("SKS", min_value=1.0, max_value=200.0, step=10.0)
        nilai_matkul = st.number_input("Nilai Mata Kuliah", min_value=0.01, max_value=100.00, step=0.10)
        kehadiran = st.number_input("Jumlah Kehadiran", min_value=1.0, max_value=20.0, step=1.0)
        tugas = st.number_input("Jumlah Tugas", min_value=1.0, max_value=20.0, step=1.0)
        penilaian_dosen = st.number_input("Skor Penilaian Dosen", min_value=1.0, max_value=5.00, step=0.1)
        waktu_penyelesaian = st.number_input("Waktu Penyelesaian", min_value=1.0, max_value=5.0, step=1.0)
        ...
        if submitted:
            ...
            st.warning("⚠ Harap lengkapi seluruh field.")